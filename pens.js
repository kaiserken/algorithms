...
var A = [100,100,5,1,1,2,2,3,5,5,5,5,1];
var X = 5;
...
function solution(A, X){
  ...
  for (var key in wood){
    if (wood[key] < 2){delete wood[key];}
    if (wood[key]>=4 && parseInt(key)*parseInt(key) >= X){
      possible+=1;
    }
    for(var key1 in wood){
      if (wood[key1] < 2){delete wood[key1];}
      if (key !== key1 && !used[key+key1] && !used[key1+key] && (parseInt(key)*parseInt(key1)) >= X){
        possible +=1;
        used[key+key1] = ++used[key+key1]|| 1;
        used[key1+key] = ++used[key1+key]|| 1;
      }
    }
  }
  console.log(used);
  console.log(wood);
  if (possible > 1000000000){return -1;}
  return possible;
